...
master_df = pd.read_sql("SELECT Stoptime, `End.station.id` FROM `citibike.2019`;", con=conn)
master_df = master_df.rename(columns={"Stoptime": "datetime", "End.station.id": "station_id"})
conn.close()
...
trip_counts = master_df.groupby(['station_id', 'month', 'day', 'hour']).size().reset_index(name='counts')
